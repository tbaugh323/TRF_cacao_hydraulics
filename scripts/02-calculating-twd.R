...
for (i in 1:length(dendro_files)) {
  ...
  current_sensor <- current_sensor |> 
    filter(TIME >= as.POSIXct("2026-06-22 10:00:00", tz = "America/Phoenix"))
  ...
  current_sensor_full <- current_sensor_twd |> 
    ...
    dplyr::select(date, datetime, daily_max, daily_min, amplitude, twd, twd_norm, twd_min, instance, Temperature, Humidity, VPD, Tree_ID, minute, hour, displacement.um.fixed, hour, minute, second)
