...
clean_all_data |> 
  filter(Date == as.Date("2026-07-23")) |>
...
sum_by_canopy |> 
  ...
  ggplot(aes(x = dt, y = PhiPS2_m)) +
  geom_vline(data = rain_df, aes(xintercept = dt), linetype = 2, linewidth = 1, color = "royalblue4") +
  # geom_line(aes(x = dt, y = PhiPS2_m, group = interaction(Date, canopy), color = factor(individual)), alpha = 0.5, position = position_dodge(width = 5000)) +
  geom_point(aes(x = dt, y = PhiPS2_m, color = canopy, shape = canopy), position = position_dodge(width = 5000), size = 3) +
  scale_color_manual(values = c("skyblue2", "tomato")) +
  labs(x = "Date time",
       color = "Individual",
       shape = "Canopy level") +
...
sum_by_canopy |> 
  filter(period != "afternoon") |> 
  ggplot(aes(x = factor(period, levels = c("early", "morning", "midday", "afternoon")), y = PhiPS2_m, group = interaction(factor(period), canopy), color = canopy)) +
  geom_boxplot() +
  # geom_jitter(width = 0.15, size = 2, alpha = 0.3) +
  scale_color_manual(values = c("skyblue2", "tomato")) +
  labs(x = "Period") +
...
sum_by_canopy |> 
  mutate(condition = factor(condition, levels = c("predrought", "drought", "recovery"))) |> 
  ggplot(aes(x = condition, y = PhiPS2_m, color = canopy)) +
  geom_boxplot() +
  # geom_jitter(width = 0.15, size = 2, alpha = 0.3) +
  scale_color_manual(values = c("skyblue2", "tomato")) +
  labs(y = "Maximum fluorescence in light",
