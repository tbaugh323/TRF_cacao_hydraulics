...
hydroscape_coords_u <- data.frame(hydroscape$PD_m, hydroscape$MD_m, hydroscape$canopy) |> 
  filter(hydroscape.canopy == "Upper canopy") |> select(-hydroscape.canopy)
hydroscape_poly_u <- Polygon(hydroscape_coords_u, hole = F)
hydroscape_area_u <- hydroscape_poly_u@area
hydroscape_area_u

hydroscape_coords_l <- data.frame(hydroscape$PD_m, hydroscape$MD_m, hydroscape$canopy) |> 
  filter(hydroscape.canopy == "Lower canopy") |> select(-hydroscape.canopy)
hydroscape_poly_l <- Polygon(hydroscape_coords_l, hole = F)
hydroscape_area_l <- hydroscape_poly_l@area
hydroscape_area_l

# Lower canopy area: 0.44125
# Upper canopy area: 0.516875
...
wp_PDMD |> 
  ...
  annotate("text", x = -2, y = -0.75, label = paste("2nd level slope: ", round(regress_2$coefficients[2], 2)), size = 4.5, color = "mediumseagreen") +
  annotate("text", x = -2, y = -1, label = paste("3rd level slope: ", round(regress_3$coefficients[2], 2)), size = 4.5, color = "orchid3") +
  annotate("text", x = -2, y = -1.25, label = paste("Upper area: ", round(hydroscape_area_u, 2)), size = 4.5, color = "tomato") +
  annotate("text", x = -2, y = -1.5, label = paste("Lower area: ", round(hydroscape_area_l, 2)), size = 4.5, color = "skyblue2") +
  geom_abline(aes(slope = 1, intercept = 0), linetype = "dashed", linewidth = 1) +
  ...
  geom_abline(aes(slope = regress_3$coefficients[2], intercept = regress_3$coefficients[1]), color = "palevioletred") +
  geom_abline(aes(slope = regress_2$coefficients[2], intercept = regress_2$coefficients[1]), color = "seagreen") +
  geom_polygon(data = hydroscape, aes(x = PD_m, y = MD_m, fill = canopy), alpha = 0.2) +  # fill = canopy
