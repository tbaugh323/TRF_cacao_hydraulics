...
for (i in 1:length(all_600_files)) {
  ...
  # If the file got fucked up and had a million rows, delete them
  if (length(colnames(hold)) > 200) {
    hold <- hold[,-(109:378)]
  }
  
  # If the file got fucked up and the date is wrong, fix that
  ...
  hold_clean <- hold_clean |> 
    mutate(dt = as.POSIXct(paste(Date, Time)),
           period = case_when(Time >= hms::as_hms("5:00:00") & Time < hms::as_hms("7:00:00") ~ "early",
